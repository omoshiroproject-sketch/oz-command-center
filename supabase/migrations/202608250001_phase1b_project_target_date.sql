-- OZ COMMAND CENTER Phase 1B project target dates
-- Add nullable formal project target dates and extend only the reviewed PROJECT_CREATE path.
-- Existing migrations, RLS policies, table grants, and formal data remain unchanged.

alter table public.projects add column if not exists target_date date null;

create index if not exists projects_owner_target_date_idx
  on public.projects(owner_id, target_date)
...
begin
  ...
  if p_decision = 'NEEDS_EDIT' then
    ...
    v_project_target_date := null;
    if nullif(v_candidate->>'targetDate', '') is not null then
      if (v_candidate->>'targetDate') !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then
        raise exception 'PROJECT_TARGET_DATE_INVALID' using errcode = '22023';
      end if;
      begin
        v_project_target_date := (v_candidate->>'targetDate')::date;
      exception when invalid_datetime_format or datetime_field_overflow then
        raise exception 'PROJECT_TARGET_DATE_INVALID' using errcode = '22023';
      end;
      if to_char(v_project_target_date, 'YYYY-MM-DD') <> v_candidate->>'targetDate' then
        raise exception 'PROJECT_TARGET_DATE_INVALID' using errcode = '22023';
      end if;
    end if;
    ...
    insert into public.projects(owner_id, slug, name, description, status, importance, target_date)
    values(
      v_owner,
